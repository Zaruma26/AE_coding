...
num_files = 10;
file_prefix = 'datos_AE_23_cod_fdsk_';  % Prefijo del nombre del archivo
file_prefix2 = 'datos_AE_33_cod_fdsk_';  % Prefijo del nombre del archivo
file_prefix3 = 'datos_AE_43_cod_fdsk_';  % Prefijo del nombre del archivo
file_prefix4 = 'datos_AE_63_cod_fdsk_';  % Prefijo del nombre del archivo
...
for i = 1:num_files
    ...
    ber_matrix4(:, i) = data4{i}.Var2;  % Suponiendo que la segunda columna es BLER
    ber_cal(:, i) = 1-(1-data{i}.Var2).^(1/2);
    ber_cal2(:, i) = 1-(1-data2{i}.Var2).^(1/3);
    ber_cal3(:, i) =1-(1-data3{i}.Var2).^(1/4);
    ber_cal4(:, i) =1-(1-data4{i}.Var2).^(1/6);
end
...
ylabel('BLER');
title('Lineal');
legend(sprintf('AE (%d,%d)',2,3),sprintf('AE (%d,%d)',3,3),sprintf('AE (%d,%d)',4,3),sprintf('AE (%d,%d)',6,3))
grid on;
...
pskBLER = 1-(1-berawgn(EbNodB,'psk',8,'nondiff')).^3;
%pskBLER2 = 1-(1-berawgn(EbNodB,'psk',4,'nondiff')).^2;
semilogy(EbNodB,pskBLER,'r--','LineWidth',2)
%semilogy(EbNodB,pskBLER2,'m--','LineWidth',2)
hold off
legend(sprintf('AE (%d,%d)',2,3),sprintf('AE (%d,%d)',3,3),sprintf('AE (%d,%d)',4,3),sprintf('AE (%d,%d)',6,3),sprintf('8PSK (%d,%d)',3,3),'Location','southwest')


%%%%% GRAFICA BER
figure;
sgtitle('BER vs Eb/No (8PSK)');
subplot(1,2,1);
hold on;
...
% Calculate uncoded block error rate (R=k/n=1)
pskBER = berawgn(EbNodB,'psk',8,'nondiff');
%pskBLER2 = 1-(1-berawgn(EbNodB,'psk',4,'nondiff')).^2;
