...
num_files = 10;
file_prefix = 'datos_AE_25_cod_fdsk_';  % Prefijo del nombre del archivo
file_prefix2 = 'datos_AE_55_cod_fdsk_';  % Prefijo del nombre del archivo
file_prefix3 = 'datos_AE_65_cod_fdsk_';  % Prefijo del nombre del archivo
file_prefix4 = 'datos_AE_105_cod_fdsk_';  % Prefijo del nombre del archivo
...
for i = 1:num_files
    ...
    ber_cal2(:, i) = 1-(1-data2{i}.Var2).^(1/5);
    ber_cal3(:, i) =1-(1-data3{i}.Var2).^(1/6);
    ber_cal4(:, i) =1-(1-data4{i}.Var2).^(1/10);
end
...
ylabel('BLER');
title('Lineal');
legend(sprintf('AE (%d,%d)',2,5),sprintf('AE (%d,%d)',5,5),sprintf('AE (%d,%d)',6,5),sprintf('AE (%d,%d)',10,5));
grid on;
...
pskBLER = 1-(1-berawgn(EbNodB,'psk',2^k,'nondiff')).^6;
semilogy(EbNodB,pskBLER,'r--','LineWidth',2)
hold off
legend(sprintf('AE (%d,%d)',2,5),sprintf('AE (%d,%d)',5,5),sprintf('AE (%d,%d)',6,5),sprintf('AE (%d,%d)',10,5),sprintf('32PSK (%d,%d)',n,k),'Location','southwest')

% Graficar BER
figure;
sgtitle('BER vs Eb/No (32PSK)');
subplot(1,2,1);
hold on;
