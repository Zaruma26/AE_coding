...
num_files = 10;
file_prefix = 'datos_AE_24_cod_fdsk_';  % Prefijo del nombre del archivo
file_prefix2 = 'datos_AE_44_cod_fdsk_';  % Prefijo del nombre del archivo
file_prefix3 = 'datos_AE_64_cod_fdsk_';  % Prefijo del nombre del archivo
file_prefix4 = 'datos_AE_84_cod_fdsk_';  % Prefijo del nombre del archivo
...
hold off
legend(sprintf('AE (%d,%d)',2,4),sprintf('AE (%d,%d)',4,4),sprintf('AE (%d,%d)',6,4),sprintf('AE (%d,%d)',8,4),sprintf('16PSK (%d,%d)',n,k),'Location','southwest')


%%%% Grafica BER
figure;
sgtitle('BER vs Eb/No (16PSK)');
subplot(1,2,1);
hold on;
...
% Calculate uncoded block error rate (R=k/n=1)
pskBER = berawgn(EbNodB,'psk',16,'nondiff');
