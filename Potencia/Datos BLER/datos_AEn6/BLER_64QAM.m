...
num_files = 10;
file_prefix = 'datos_AE_2_6_10dB_sk_cod_fd_';  % Prefijo del nombre del archivo
file_prefix2 = 'datos_AE_6_6_10dB_sk_cod_fd_';  % Prefijo del nombre del archivo
file_prefix3 = 'datos_AE_9_6_10dB_sk_cod_fd_';  % Prefijo del nombre del archivo
file_prefix4 = 'datos_AE_12_6_10dB_sk_cod_fd_';  % Prefijo del nombre del archivo
...
% Calcular la media y el intervalo de confianza del 95 Tasa (2,6)%
...
% Calcular la media y el intervalo de confianza del 95 Tasa (6,6)%
...
% Calcular la media y el intervalo de confianza del 95 Tasa (9,6)%
...
% Calcular la media y el intervalo de confianza del 95 Tasa (12,6)%
...
ylabel('BLER');
title('Lineal');
legend(sprintf('AE (%d,%d)',2,6),sprintf('AE (%d,%d)',6,6),sprintf('AE (%d,%d)',9,6),sprintf('AE (%d,%d)',12,6))
grid on;
...
hold off
legend(sprintf('AE (%d,%d)',2,6),sprintf('AE (%d,%d)',6,6),sprintf('AE (%d,%d)',9,6),sprintf('AE (%d,%d)',12,12),sprintf('64QAM (%d,%d)',n,k),'Location','southwest')


%%%% Grafica BER
figure;
sgtitle('BER vs Eb/No (64QAM)');
subplot(1,2,1);
hold on;
...
% Calculate uncoded block error rate (R=k/n=1)
qamBER = berawgn(EbNodB,'qam',64,'nondiff');
semilogy(EbNodB,qamBER,'r--','LineWidth',2)
hold off
legend(sprintf('AE (%d,%d)',2,6),sprintf('AE (%d,%d)',6,6),sprintf('AE (%d,%d)',9,6),sprintf('AE (%d,%d)',12,6),sprintf('64QAM (%d,%d)',n,k),'Location','southwest')
